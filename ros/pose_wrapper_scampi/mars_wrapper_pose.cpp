  ...
  , m_sett_(nh)
{
  ...
  pose1_sensor_sptr_->use_dynamic_meas_noise_ = true;
  ...
  pub_core_pose_state_center_ = nh.advertise<geometry_msgs::PoseStamped>("core_pose_state_center_out", m_sett_.pub_cb_buffer_size_);
...
void MarsWrapperPose::ImuMeasurementCallback(const sensor_msgs::ImuConstPtr& meas)
{
  ...
  Time timestamp;
  static double start_time = meas->header.stamp.toSec();
...
*/
  ...
  timestamp = Time(start_time+counter*(1./400));
  counter++;
  ...
  //if (!core_logic_.core_is_initialized_ && b_have_pos_)
...
{
  std::cout << "Update: " << meas->header.stamp.toSec() << std::endl;
...
void MarsWrapperPose::RunCoreStatePublisher()
{
  ...
  // Publishing PoseSensor pose in world.
  //mars::PoseSensorStateType pose_cal = sensor_sptr->get_state(pose1_sensor_sptr_->initial_calib_);

  mars::PoseSensorStateType pose_cal  = pose1_sensor_sptr_.get()->get_state(pose1_sensor_sptr_->initial_calib_);

  Eigen::Vector3d p_ip           = pose_cal.p_ip_;
  Eigen::Quaternion<double> q_ip = pose_cal.q_ip_;

  Eigen::Vector3d p_wi           = latest_core_state.p_wi_;
  Eigen::Quaternion<double> q_wi = latest_core_state.q_wi_;

  Eigen::Quaternion<double> q_wp = q_wi * q_ip;
  Eigen::Vector3d p_wp = p_wi + q_wi.toRotationMatrix()*p_ip;

  mars::PoseSensorStateType state;
  state.p_ip_ = p_wp;
  state.q_ip_ = q_wp;

  pub_core_pose_state_center_.publish(
      MarsMsgConv::PoseStateToPoseMsg(latest_state.timestamp_.get_seconds(), state));
